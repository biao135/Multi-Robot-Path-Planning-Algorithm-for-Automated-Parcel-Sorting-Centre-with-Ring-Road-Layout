...
public class Vertex
{
    ...
    public void setCurveGridPosition(float[] curveGridPosition)
    {
        this.curveGridPosition = curveGridPosition;
    }
    public float[] getCurveGridPosition()
    {
    ...
    public void setIsBuffer(bool isBuffer)
    {
    ...
    public void setRingRoadPair(Vertex pairedVertex)
    {
        this.pairedVertex = pairedVertex;
    }
    public Vertex getRingRoadPair()
    {
        return pairedVertex;
    }
}
...
public class Curve
{
    ...
    public Curve(int label, Vertex vertexStart, Vertex vertexEnd, float[] center, float startAngle, float endAngle, float radius)
    {
        this.label = label;
...
public class Graph
{
    ...
    public List<Vertex> findRouteFromStartToEntrance(Vertex vertexStart, Vertex destination, int changeDirectionPenalize = 10)
    {
        ...
        {
            toVisit = (vertexStart as MoveGrid).getEdge();
        }
        else
        {
            toVisit = (vertexStart as ChargingPoint).getEdge();
        }
        ...
        while (toVisit.Count != 0)
        {
            ...
            if (nextVertexToVisit.getRingRoadPair() == destination)
            {
                curRoute.Add(nextVertexToVisit.getRingRoadPair());
                return curRoute;
            }
    ...
    public List<Vertex> findRouteInRingRoad(Vertex vertexStart, Vertex destination)
    {
        ...
        while (toVisit.Count != 0)
        {
            ...
            if (nextVertexToVisit.getIsCurveStart() && nextVertexToVisit.getIsCorner())
            {
                curRoute.AddRange(nextVertexToVisit.getCurve().getCurveGrids());
                nextVertexToVisit = nextVertexToVisit.getCurve().getVertexEnd();
                if (nextVertexToVisit == destination)
                {
                    curRoute.AddRange(nextVertexToVisit.getCurve().getCurveGrids());
                    return curRoute;
    ...
    public List<Vertex> findRouteWithOneDestination(Vertex vertexStart, List<Vertex> destination, int changeDirectionPenalize = 10)
    {
        ...
        while (toVisit.Count != 0)
        {
            ...
            {
                Edge currentEdge = newEdges[j];
                if (!visited.Contains(currentEdge.getNextVertex()) && !currentEdge.getNextVertex().getIsOuterRingRoad())
                {
                    toVisit.Add(currentEdge);
                    int newCost = curCost + currentEdge.getCost();
                    if (lastDirection != currentEdge.getDirection() || currentEdge.getNextVertex().getIsOuterRingRoad())
