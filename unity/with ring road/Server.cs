...
public class Server : Graph
{
    ...
    private int singleAreaThreshold = 0;
    ...
    private List<int> areaInRingRoadCount = new List<int>();

    public Server(int[,] graph, int[,] ringRoad, TextMeshPro throughputCounter, TextMeshPro robotCounter) : base(graph, ringRoad){
        // initialize map
        ...
        //singleAreaThreshold = (int)(maximumRobotInRingRoad * 0.3);
...
public void addArea(Area area)
    ...
    private float[] calculatePositionInCircle(Curve curve, float angle)
    {
        angle = (float)(angle / 180 * Math.PI);
        float[] curveCenter = curve.getCenter();
        float curveRadius = curve.getRadius();
        float posX = (float)(curveCenter[0] + Math.Sin(angle) * curveRadius);
        float posY = (float)(curveCenter[1] + Math.Cos(angle) * curveRadius);
        return new float[] { posX, posY };
    }

    public void addCurve(int label, int[] startVertexPosition, int[] endVertexPosition, float[] center, float initialAngle, float endAngle, float radius, bool entrance)
    {
        ...
        float angle = initialAngle > endAngle ? initialAngle - incrementAngle : initialAngle + incrementAngle;
        while (true)
        {
            float[] gridPosition = calculatePositionInCircle(curve, angle);
    ...
    public void initializeOuterRingRoad()
    {
        ...
        for (int i = 0; i <= shape[0]; i++)
        {
            for (int j = 0; j <= shape[1]; j++)
            {
                if (ringRoad[i, j] == 4)
                {
                    (vertices[i][j] as MoveGrid).clearEdge();
                    if (ringRoad[i - 1, j] == 2)
                    {
                        vertices[i][j].setIsEntrance(true);
                        vertices[i - 1][j].setIsExit(true);
                        vertices[i][j].setRingRoadPair(vertices[i - 1][j]);
                        vertices[i - 1][j].setRingRoadPair(vertices[i][j]);
                        (vertices[i][j] as MoveGrid).addEdge(vertices[i + 1][j], 3, 2);
                    }
                    else if (ringRoad[i - 1, j] == 3)
                    {
                        vertices[i][j].setIsExit(true);
                        vertices[i - 1][j].setIsEntrance(true);
                        vertices[i][j].setRingRoadPair(vertices[i - 1][j]);
                        vertices[i - 1][j].setRingRoadPair(vertices[i][j]);
                        (vertices[i + 1][j] as MoveGrid).addEdge(vertices[i][j], 3, 1);
                    }
                    else if (ringRoad[i + 1, j] == 2)
                    {
                        vertices[i][j].setIsEntrance(true);
                        vertices[i + 1][j].setIsExit(true);
                        vertices[i][j].setRingRoadPair(vertices[i + 1][j]);
                        vertices[i + 1][j].setRingRoadPair(vertices[i][j]);
                        (vertices[i][j] as MoveGrid).addEdge(vertices[i - 1][j], 3, 1);
                    }
                    else if (ringRoad[i + 1, j] == 3)
                    {
                        vertices[i][j].setIsExit(true);
                        ...
                        vertices[i][j].setRingRoadPair(vertices[i + 1][j]);
                        vertices[i + 1][j].setRingRoadPair(vertices[i][j]);
                        (vertices[i - 1][j] as MoveGrid).addEdge(vertices[i][j], 3, 2);
                    }
                    else if (ringRoad[i, j - 1] == 2)
                    {
                        vertices[i][j].setIsEntrance(true);
                        vertices[i][j - 1].setIsExit(true);
                        vertices[i][j].setRingRoadPair(vertices[i][j - 1]);
                        vertices[i][j - 1].setRingRoadPair(vertices[i][j]);
                        (vertices[i][j] as MoveGrid).addEdge(vertices[i][j + 1], 3, 4);
                    }
                    else if (ringRoad[i, j - 1] == 3)
                    {
                        vertices[i][j].setIsExit(true);
                        vertices[i][j - 1].setIsEntrance(true);
                        vertices[i][j].setRingRoadPair(vertices[i][j - 1]);
                        vertices[i][j - 1].setRingRoadPair(vertices[i][j]);
                        (vertices[i][j + 1] as MoveGrid).addEdge(vertices[i][j], 3, 3);
                    }
                    else if (ringRoad[i, j + 1] == 2)
                    {
                        vertices[i][j].setIsEntrance(true);
                        ...
                        vertices[i][j].setRingRoadPair(vertices[i][j + 1]);
                        vertices[i][j + 1].setRingRoadPair(vertices[i][j]);
                        (vertices[i][j] as MoveGrid).addEdge(vertices[i][j - 1], 3, 3);
                    }
                    else if (ringRoad[i, j + 1] == 3)
                    {
                        vertices[i][j].setIsExit(true);
                        ...
                        vertices[i][j].setRingRoadPair(vertices[i][j + 1]);
                        vertices[i][j + 1].setRingRoadPair(vertices[i][j]);
                        (vertices[i][j - 1] as MoveGrid).addEdge(vertices[i][j], 3, 4);
                    }
                }
            }
        }
    ...
    public void setExitEntrances(int areaNo, int type, int task, int[] curveStartLabel, int[] curveEndLabel)
    {
        // type:
        // 1: set exit
        // 2: set entrance
        ...
        if (curveStartLabel[0] != curveEndLabel[0])
        {
            int minI = Math.Min(curveStartLabel[0], curveEndLabel[0]);
            int maxI = Math.Max(curveStartLabel[0], curveEndLabel[0]);
            for (int i = minI; i < maxI; i++)
            {
                Vertex currentVertex = findVertex(new int[] { i, curveStartLabel[1] });
                if (type == 1 && task == 1)
                {
        ...
        {
            int minI = Math.Min(curveStartLabel[1], curveEndLabel[1]);
            int maxI = Math.Max(curveStartLabel[1], curveEndLabel[1]);
            for (int i = minI; i < maxI; i++)
            {
                Vertex currentVertex = findVertex(new int[] { curveStartLabel[0], i });
                if (type == 1 && task == 1)
                {
                    area.addPickUpExits(currentVertex);
                    ringRoadPickUpExits[area.getLabel() - 1].Add(0);
    ...
    public Vertex requestRingRoadExitEntrance(Vertex vertex, int type, int task)
    {
        ...
        if (type == 1 && task == 1)
        {
            int minValue = ringRoadPickUpExits[area.getLabel() - 1].Min();
            List<int> minIndexes = new List<int>();
            for (int i = 0; i < ringRoadPickUpExits[area.getLabel() - 1].Count(); i++)
            {
                if (ringRoadPickUpExits[area.getLabel() - 1][i] == minValue)
                {
                    minIndexes.Add(i);
                }
            }
            int[] currentLabel = ringRoadPickUpExitsVertex[area.getLabel() - 1][minIndexes[0]].getLabel();
            int minDistance = Math.Abs(vertexLabel[0] - currentLabel[0]) + Math.Abs(vertexLabel[1] - currentLabel[1]);
            int minDistanceIndex = 0;
            for (int i = 1; i < minIndexes.Count(); i++)
            {
                currentLabel = ringRoadPickUpExitsVertex[area.getLabel() - 1][minIndexes[i]].getLabel();
                int currentDistance = Math.Abs(vertexLabel[0] - currentLabel[0]) + Math.Abs(vertexLabel[1] - currentLabel[1]);
            ...
            ringRoadPickUpExits[area.getLabel() - 1][minIndexes[minDistanceIndex]] += 1;
            return ringRoadPickUpExitsVertex[area.getLabel() - 1][minIndexes[minDistanceIndex]];
        }
        else if (type == 1 && task == 2)
        {
            int minValue = ringRoadDropOffExits[area.getLabel() - 1].Min();
            List<int> minIndexes = new List<int>();
            for (int i = 0; i < ringRoadDropOffExits[area.getLabel() - 1].Count(); i++)
            {
                if (ringRoadDropOffExits[area.getLabel() - 1][i] == minValue)
                {
                    minIndexes.Add(i);
                }
            }
            int[] currentLabel = ringRoadDropOffExitsVertex[area.getLabel() - 1][minIndexes[0]].getLabel();
            int minDistance = Math.Abs(vertexLabel[0] - currentLabel[0]) + Math.Abs(vertexLabel[1] - currentLabel[1]);
            int minDistanceIndex = 0;
            for (int i = 1; i < minIndexes.Count(); i++)
            {
                currentLabel = ringRoadDropOffExitsVertex[area.getLabel() - 1][minIndexes[i]].getLabel();
                int currentDistance = Math.Abs(vertexLabel[0] - currentLabel[0]) + Math.Abs(vertexLabel[1] - currentLabel[1]);
                if (currentDistance < minDistance)
                {
                    minDistance = currentDistance;
            ...
            ringRoadDropOffExits[area.getLabel() - 1][minIndexes[minDistanceIndex]] += 1;
            return ringRoadDropOffExitsVertex[area.getLabel() - 1][minIndexes[minDistanceIndex]];
        }
        else if (type == 2 && task == 1)
        {
            int minValue = ringRoadPickUpEntrances[area.getLabel() - 1].Min();
            List<int> minIndexes = new List<int>();
            for (int i = 0; i < ringRoadPickUpEntrances[area.getLabel() - 1].Count(); i++)
            {
                if (ringRoadPickUpEntrances[area.getLabel() - 1][i] == minValue)
                {
                    minIndexes.Add(i);
                }
            }
            int[] currentLabel = ringRoadPickUpEntrancesVertex[area.getLabel() - 1][minIndexes[0]].getLabel();
            int minDistance = Math.Abs(vertexLabel[0] - currentLabel[0]) + Math.Abs(vertexLabel[1] - currentLabel[1]);
            int minDistanceIndex = 0;
            for (int i = 1; i < minIndexes.Count(); i++)
            {
                currentLabel = ringRoadPickUpEntrancesVertex[area.getLabel() - 1][minIndexes[i]].getLabel();
                int currentDistance = Math.Abs(vertexLabel[0] - currentLabel[0]) + Math.Abs(vertexLabel[1] - currentLabel[1]);
                if (currentDistance < minDistance)
                {
                    minDistance = currentDistance;
            ...
            ringRoadPickUpEntrances[area.getLabel() - 1][minIndexes[minDistanceIndex]] += 1;
            return ringRoadPickUpEntrancesVertex[area.getLabel() - 1][minIndexes[minDistanceIndex]];
        }
        else
        {
            int minValue = ringRoadDropOffEntrances[area.getLabel() - 1].Min();
            List<int> minIndexes = new List<int>();
            for (int i = 0; i < ringRoadDropOffEntrances[area.getLabel() - 1].Count(); i++)
            {
                if (ringRoadDropOffEntrances[area.getLabel() - 1][i] == minValue)
                {
                    minIndexes.Add(i);
                }
            }
            int[] currentLabel = ringRoadDropOffEntrancesVertex[area.getLabel() - 1][minIndexes[0]].getLabel();
            int minDistance = Math.Abs(vertexLabel[0] - currentLabel[0]) + Math.Abs(vertexLabel[1] - currentLabel[1]);
            int minDistanceIndex = 0;
            for (int i = 1; i < minIndexes.Count(); i++)
            {
                currentLabel = ringRoadDropOffEntrancesVertex[area.getLabel() - 1][minIndexes[i]].getLabel();
                int currentDistance = Math.Abs(vertexLabel[0] - currentLabel[0]) + Math.Abs(vertexLabel[1] - currentLabel[1]);
                if (currentDistance < minDistance)
                {
                    minDistance = currentDistance;
            ...
            ringRoadDropOffEntrances[area.getLabel() - 1][minIndexes[minDistanceIndex]] += 1;
            return ringRoadDropOffEntrancesVertex[area.getLabel() - 1][minIndexes[minDistanceIndex]];
        }
    }

    public void releaseRingRoadExitEntrance(Vertex vertex, int areaLabel, int type, int task)
    {
        // type:
        // 1: get exit
        // 2: get entrance
        ...
        if (type == 1 && task == 1)
        {
            for (int i = 0; i < ringRoadPickUpExitsVertex[areaLabel - 1].Count(); i++)
            {
                if (ringRoadPickUpExitsVertex[areaLabel - 1][i] == vertex)
                {
                    ringRoadPickUpExits[areaLabel - 1][i] -= 1;
                    break;
                }
            }
        }
        ...
        {
            for (int i = 0; i < ringRoadDropOffExitsVertex[areaLabel - 1].Count(); i++)
            {
                if (ringRoadDropOffExitsVertex[areaLabel - 1][i] == vertex)
                {
                    ringRoadDropOffExits[areaLabel - 1][i] -= 1;
                    break;
                }
            }
        }
        ...
        {
            for (int i = 0; i < ringRoadPickUpEntrancesVertex[areaLabel - 1].Count(); i++)
            {
                if (ringRoadPickUpEntrancesVertex[areaLabel - 1][i] == vertex)
                {
                    ringRoadPickUpEntrances[areaLabel - 1][i] -= 1;
                    break;
                }
            }
        }
        ...
        {
            for (int i = 0; i < ringRoadDropOffEntrancesVertex[areaLabel - 1].Count(); i++)
            {
                if (ringRoadDropOffEntrancesVertex[areaLabel - 1][i] == vertex)
                {
                    ringRoadDropOffEntrances[areaLabel - 1][i] -= 1;
                    break;
                }
            }
        }
    ...
    public ChargingPoint requestCharging(bool urgent, Robot robot)
    {
        ...
        if (urgent && availableIndexes.Count == 0)
        {
            ...
            if (chargingPointsRobots[highestIndex].getBattery() > 8640f)
            {
    ...
    public bool requestEnterRingRoad(int currentAreaLabel)
    {
    ...
    public void notifyExitRingRoad(int currentAreaLabel)
    {
        robotInRingRoadCount--;
    }

    public void setBufferArea(int[] topLeftPoint, int[] bottomRightPoint)
    {
        int minI = Math.Min(topLeftPoint[0], bottomRightPoint[0]);
        int maxI = Math.Max(topLeftPoint[0], bottomRightPoint[0]);
        int minJ = Math.Min(topLeftPoint[1], bottomRightPoint[1]);
        int maxJ = Math.Max(topLeftPoint[1], bottomRightPoint[1]);
        for (int i = minI; i <= maxI; i++)
        {
            for (int j = minJ; j <= maxJ; j++)
            {
                vertices[i][j].setIsBuffer(true);
            }
        }
    }
}