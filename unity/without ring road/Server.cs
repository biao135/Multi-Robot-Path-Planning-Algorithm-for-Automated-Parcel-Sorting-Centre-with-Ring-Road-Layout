...
public class Server : Graph
{
    ...
    private float[] calculatePositionInCircle(Curve curve, float angle){
        ...
        float posX = curveCenter[0] + (float)Mathf.Sin(angle) * curveRadius;
        float posY = curveCenter[1] + (float)Mathf.Cos(angle) * curveRadius;
    ...
    public void addCurve(int label, int[] startVertexPosition, int[] endVertexPosition, float[] center, float initialAngle, float endAngle, float radius, bool entrance){
        ...
        for(int i = 0; i < curveGrids.Count; i++){
            if(curveGrids[i].getIsCurveCenter()){
                float[] currentPosition = curveGrids[i].getCurveGridPosition();
