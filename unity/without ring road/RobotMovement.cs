...
public class RobotMovement : MonoBehaviour
{
    ...
    public void Initialize(int label, Server server, int[] position){
        this.server = server;
        ...
        robot = new Robot(label, position, server);
    ...
    private List<Vertex> findRoute(Vertex vertexStart, Vertex destinationVertex, List<Vertex> destinations, int task){
        ...
        if(Math.Abs(currentAreaLabel - nextAreaLabel) > 1 &&
            ...
            if(route1.Count == 0){
                Debug.Log("route 1: " + vertexStart.getLabel()[0] + " " + vertexStart.getLabel()[1] + " to " + requestedEntrance.getLabel()[0] + " " + requestedEntrance.getLabel()[1]);
            }
            List<Vertex> route2 = server.findRouteInRingRoad(route1.Last(), requestedExit);
            if(route2.Count == 0){
                Debug.Log("route 2: " + route1.Last().getLabel()[0] + " " + route1.Last().getLabel()[1] + " to " + requestedExit.getLabel()[0] + " " + requestedExit.getLabel()[1]);
            }
            List<Vertex> route3 = server.findRouteWithOneDestination(route2.Last(), destinations);
            if(route3.Count == 0){
                Debug.Log("route 3: " + route2.Last().getLabel()[0] + " " + route2.Last().getLabel()[1] + " to " + destinations[0].getLabel()[0] + " " + destinations[0].getLabel()[1]);
            }
    ...
    private float[] moveInCurve(Curve curve, float angle){
        ...
        float posX = curveCenter[1] + (float)(float)Math.Cos(angle) * curveRadius;
        float posY = curveCenter[0] + (float)(float)Math.Sin(angle) * curveRadius;
    ...
    void Update(){
            ...
            if(reservedGrids.Count != 0 || halfGrid){
                ...
                if(!isCurve){
                    if(halfGrid){
                        ...
                        if(pickUpPoint != null && pickUpPoint.Contains(position)){
                            displayParcel = true;
                        }
                        else if(dropOffPoint != null && dropOffPoint.Contains(position)){
        ...
        if(!halfGrid){
            if(reservedGridsCount == 0){
                if(lastSpeedLevel == 1){ // no available grids
                    speedLevel = 0;
