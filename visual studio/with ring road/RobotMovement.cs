...
public class RobotMovement
{
    ...
    private double[] moveInCurve(Curve curve, double angle)
    {
        ...
        double posX = curveCenter[1] + (double)(double)Math.Cos(angle) * curveRadius;
        double posY = curveCenter[0] + (double)(double)Math.Sin(angle) * curveRadius;
    ...
    public void Update()
    {
        ...
        if (speedLevel > 0)
        {
            ...
            else
            {
                ...
                {
                    currentAngle = (double)Math.Min(currentAngle + angularSpeed * fixedDeltaTime * (speedLevel + 0.5f) / 6f, endAngle);
                }
                else if (currentAngle > stopAngle)
                {
                    currentAngle = (double)Math.Max(currentAngle - angularSpeed * fixedDeltaTime * (speedLevel + 0.5f) / 6f, endAngle);
                }
